...
if __name__ == "__main__":
    ...
    path = 'D:\\东北证券NET专业版\\userdata_mini'
    # session_id为会话编号，策略使用方对于不同的Python策略需要使用不同的会话编号
    session_id = 1
    xt_trader = XtQuantTrader(path, session_id)
    # 创建资金账号为1000000365的证券账号对象
    acc = StockAccount('40158111', 'STOCK')
    ...
    fix_result_order_id = xt_trader.order_stock(acc, stock_code, xtconstant.STOCK_BUY, 100, xtconstant.FIX_PRICE, 4.0, 'strategy_name', 'remark')
    ...
    async_seq = xt_trader.order_stock_async(acc, stock_code, xtconstant.STOCK_BUY, 100, xtconstant.FIX_PRICE, 4.088, 'strategy_name', 'remark')
    print(async_seq)
    A.go=1
    # 查询证券资产
    print("query asset:")
