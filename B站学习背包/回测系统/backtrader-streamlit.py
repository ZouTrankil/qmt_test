...
with st.form("my_form"):
    ...
    columns_val = st.selectbox("选择因子",
                               ['volume', '成交额(千元)', '换手率(%)', '量比', '市盈率(静态)', '市盈率(TTM)',
                                '市盈率(动态)', '市净率',
                                '市销率', '市销率(TTM)', '股息率(%)', '股息率(TTM)(%)', '总股本(万股)',
                                '流通股本(万股)',
    ...
    if submitted:
        ...
        for i in time_id:
            buy_list = \
            stock_daily[stock_daily["date"] == i].sort_values(by=columns_val, ascending=sort0).head(first_num)[
                "股票代码"].tolist()
...
with st.form("my_form_muti"):
    ...
    columns_muti_val = st.multiselect("选择因子_muti",
                                      ['volume', '成交额(千元)', '换手率(%)', '量比', '市盈率(静态)', '市盈率(TTM)',
                                       '市盈率(动态)', '市净率',
                                       '市销率', '市销率(TTM)', '股息率(%)', '股息率(TTM)(%)', '总股本(万股)',
                                       '流通股本(万股)',
                                       '总市值(万元)', '流通市值(万元)'], default=['总市值(万元)', '市盈率(静态)'],
                                      max_selections=2)
    ...
    if submitted:
        ...
        for i in time_id:
            factor1_stock = \
            stock_daily[stock_daily["date"] == i].sort_values(by=columns_muti_val[0], ascending=sort0_muti)[
                "股票代码"].tolist()
            factor2_stock = \
            stock_daily[stock_daily["date"] == i].sort_values(by=columns_muti_val[1], ascending=sort1_muti)[
                "股票代码"].tolist()
